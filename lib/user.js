...
exports.list = (cb) => {
  ...
  if (typeof(db) === undefined){
    cb('Error:cannot connect to db');
  }

  else{
    for(var i = 0; i < db.length; i++){
      var dbConts = {name: db[i].name, pass: db[i].pass, uid: db[i].uid, admin: db[i].admin};
      array.push(dbconts);
    }
  }
  cb('success', array);
...
exports.add = (u, cb) => {
  var usr;
  if (u in db){
    cb("Error there is a duplicate value");
  }

  else{
     usr = user(u.name,u.pass,u.admin);
     u.uid = usr.uid;
     db.push(usr);
   }
   cb("success", usr);
