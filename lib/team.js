...
var team = [
	...
	member('jdoe', 'John', 'Doe', 'senior', 'test'),
	member('mradford', 'Michael', 'Radford', 'senior', 'Michael Radford is a part of the blujay frontend team. He will be working on the implementation side of the frontend, making sure that everything that the user sees in the browser looks and works like the design specifies.'),
	member('jjsherma', 'Josh', 'Sherman', 'senior', 'Joshua Sherman is one of three members on the blujay frontend team. As a member of the frontend team, his focus will be on both design and implementation. Joshua is an experienced frontend web developer, having spent three months working in that area this past summer. He also has experience developing software with groups of varying sizes.'),
	member('henryshepher', 'Henry', 'Shepherd', 'senior', 'Henry Shepherd is part of the blujay backend team. His responsibility will be to work on handling search queries. This past summer Henry contributed to some backend development, in Java, at his internship.'),
	member('dgandle', 'Doug', 'Gandle', 'junior', 'Douglas Gandle is part of the blujay frontend team. His focus will be on the design and usability of the site, working with Michael and Joshua to create a seamless user experience. Douglas spent the last summer learning front-end web development,  and studied electronic media & design at Rensselaer Polytechnic Institute in the past.'),
	member('algentile', 'Anthony', 'Gentile', 'junior', 'Anthony Gentile is a member on the blujay backend team, his responsibilities are to handle the queried JSON objects, set up and manage the database. He will also write scripts to run data analytics on the queried JSON objects to achieve optimal performance from the web application. '),
...
function copy(member) {
	// TODO
	var memberCopy = JSON.parse(JSON.stringify(member));
	return memberCopy;
 ...
 *ng a function as a parameter javascript
...
function one(user) {
	...
	if(typeof user === 'string' || user instanceof String){
		...
		var userSearch = find(user);
		if(userSearch === null){
			return result(false, 'team member not found' , userArray);
		}

		else{
	     	userArray.push(userSearch);
			return result(true, 'team member found', userArray);
	    }
	}

	else return result(false, 'argument passed is not of type string', userArray);
}
